...
arcpy.env.workspace = r"C:\arcgisserver\directories\arcgisjobs"
projectFolder = r"C:\arcgisserver\directories\arcgisoutput"
...
def runIDW(wells, idwK, projectGDBPath):
	...
	del inPointFeatures, field, powerK
...
def runFeatToRast(tracts, projectGDBPath):
	...
	inFeatures = tracts
	field = "canrate"
	canrateRasterPath = os.path.join(projectGDBPath, "canrate")
	# cellSize =

	# Execute Feature to Raster
	canrateRaster = arcpy.FeatureToRaster_conversion(inFeatures, field, canrateRasterPath, "")
	# Get Feature to Raster Messages
	arcpy.AddMessage("\n" + arcpy.GetMessages())

	# Add the output to the MXD
	arcpy.SetParameterAsText(6, canrateRaster)
...
def runGenerateHexbins(counties, hexSize, hexUnit, projectGDBPath):
	...
	# Add the output to the MXD
	arcpy.SetParameterAsText(7, hexBins_lyr)
...
# http://desktop.arcgis.com/en/arcmap/10.6/tools/spatial-analyst-toolbox/zonal-statistics-as-table.htm
def runZonalStatsCanRate(canrateRaster, hexBins_lyr, projectGDBPath):
	#
	...
	# Delete local variables
	del canrateRaster, inZoneData, zoneField, inValueRaster, outTableName, outTable
...
def main():
	...
	canrateRaster = runFeatToRast(tracts, projectGDBPath)
	hexBins_lyr = runGenerateHexbins(counties, hexSize, hexUnit, projectGDBPath)
	runZonalStatsNitrate(nitrateIDW, hexBins_lyr, projectGDBPath)
	runZonalStatsCanRate(canrateRaster, hexBins_lyr, projectGDBPath)
	runPrefixNitrateZSat(projectGDBPath)
	runPrefixCanrateZSat(projectGDBPath)
	runJoinTables(hexBins_lyr, projectGDBPath)
	runOLS(hexBins_lyr, projectFolder, projectGDBPath)
